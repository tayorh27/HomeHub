...
public class HighPerformanceActivity extends AppCompatActivity implements SettingsCheckListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_high_performance);

        recyclerView = (RecyclerView)findViewById(R.id.recyclerView_high);
        adapter = new HubSettingsAdapter(HighPerformanceActivity.this,this,"high");
        recyclerView.setLayoutManager(new LinearLayoutManager(HighPerformanceActivity.this));
        recyclerView.setAdapter(adapter);
        customData = MyApplication.getWritableDatabase().getAllMyPosts();
        adapter.FillRecyclerView(customData);
        database = new ModeApplianceDatabase(HighPerformanceActivity.this);
        high = database.getHigh();
    }
    ...
    private void displayDialog() {
        AlertDialog alertDialog = new AlertDialog.Builder(HighPerformanceActivity.this).create();
    ...
    public void onSettingsRelativeCheckListener(CheckBox checkBox, boolean checked, int position) {
        ...
        if(checkBox.isChecked()){
            ...
            MyApplication.getWritableDatabase().updateDatabase(current.id,"high", "false");
        }else {
            checkBox.setChecked(true);
            MyApplication.getWritableDatabase().updateDatabase(current.id,"high", "true");
        }
    }
}
